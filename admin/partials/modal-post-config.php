<?php
/**
 * Modal: post type field configuration.
 *
 * @since 1.0.0
...
?>
<div id="flowbridge-modal-post" class="flowbridge-modal" style="display: none;">
		...
		<div class="flowbridge-modal-body">
			<div class="flowbridge-modal-section">
				...
				<div class="flowbridge-events-checkboxes">
					<label><input type="checkbox" name="events[]" value="post.created" /> <?php esc_html_e( 'Created', 'flowbridge-for-n8n' ); ?></label>
					<label><input type="checkbox" name="events[]" value="post.updated" /> <?php esc_html_e( 'Updated', 'flowbridge-for-n8n' ); ?></label>
					<label><input type="checkbox" name="events[]" value="post.deleted" /> <?php esc_html_e( 'Deleted', 'flowbridge-for-n8n' ); ?></label>
					<label><input type="checkbox" name="events[]" value="post.status_changed" /> <?php esc_html_e( 'Status Changed', 'flowbridge-for-n8n' ); ?></label>
				</div>
			</div>

			<div class="flowbridge-modal-section">
				<h4><?php esc_html_e( 'Sample Post', 'flowbridge-for-n8n' ); ?></h4>
				<p class="description">
					<?php esc_html_e( 'Select a sample post to detect available meta fields.', 'flowbridge-for-n8n' ); ?>
				</p>
				<select class="flowbridge-sample-selector" data-load-action="flowbridge_n8n_load_post_fields">
					<option value=""><?php esc_html_e( '-- Select a post --', 'flowbridge-for-n8n' ); ?></option>
				</select>
					...
					<table class="flowbridge-fields-table">
						...
						<tbody>
							<tr class="flowbridge-fields-empty">
								<td colspan="6"><?php esc_html_e( 'Load fields by selecting a sample post above, or save with default columns.', 'flowbridge-for-n8n' ); ?></td>
							</tr>
						</tbody>
					</table>
				</div>
